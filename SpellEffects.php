  ...
  <div class="holy-grail-grid">
      ...
      <div class="holy-grail-middle">
        ...
        <?php
          ...
          //Parses "SpellEffects" Excel doc
          if ( $xlsx = SimpleXLSX::parse('SpellEffects.xlsx') ) {
            $c = 0;
            foreach( $xlsx->rows() as $r ) {
              $Name[$c]=$r[0];
              $ShortDescription[$r[0]]=$r[1];
              $AREA[$r[0]]=$r[2];
              $RANGE[$r[0]]=$r[3];
              $DURATION[$r[0]]=$r[4];
              $DEFENSE[$r[0]]=$r[5];
              $LongDescription[$r[0]]=$r[6];
              $ScalingEffectDescription[$r[0]]=$r[7];
              $ScalingEffect[$r[0]]=array($r[8],$r[9],$r[10],$r[11],$r[12],$r[13],$r[14],$r[15],$r[16],$r[17]);
              $Note1Description[$r[0]]=$r[18];
              $Note1[$r[0]]=array($r[19],$r[20],$r[21],$r[22],$r[23],$r[24],$r[25],$r[26],$r[27],$r[28]);
              $Note2Description[$r[0]]=$r[29];
              $Note2[$r[0]]=array($r[30],$r[31],$r[32],$r[33],$r[34],$r[35],$r[36],$r[37],$r[38],$r[39]);
              $Note3Description[$r[0]]=$r[40];
              $Note3[$r[0]]=array($r[41],$r[42],$r[43],$r[44],$r[45],$r[46],$r[47],$r[48],$r[49],$r[50]);
              $c = $c+1;
            }
          }
          else {
          ...
          //if directed to a specific effect, show that effects info
          if ($_GET) {
              $Effect=filter_var(urldecode($_SERVER['QUERY_STRING']), FILTER_SANITIZE_STRING);
              echo '<p><b>'.$Effect.'</b></p>';
              echo '<p> Components: EFFECT, '.$AREA[$Effect].', '.$RANGE[$Effect].', '.$DURATION[$Effect].'</p>';
              echo '<p> DEFENSE: '.$DEFENSE[$Effect].'</p>';
              echo '<p>'.keywords($LongDescription[$Effect], $KeywordName, $KeywordCode, $KeywordLink).'</p>';
              echo '<p>';
              echo '<table border="1" cellpadding="3" style="border-collapse: collapse">';
              echo '<tr>';
              echo '<td>Energy</td>';
              echo '<td>'.$ScalingEffectDescription[$Effect].'</td>';
              if ($Note1Description[$Effect]<>"") {
                echo '<td>'.$Note1Description[$Effect].'</td>';
              }
              if ($Note2Description[$Effect]<>"") {
                echo '<td>'.$Note2Description[$Effect].'</td>';
              }
              if ($Note3Description[$Effect]<>"") {
                echo '<td>'.$Note3Description[$Effect].'</td>';
              }
              echo '</tr>';
              $c=1;
              foreach ($ScalingEffect[$Effect] as $value) {
                echo '<tr>';
                echo '<td>'.$c.'</td>';
                echo '<td>'.$ScalingEffect[$Effect][$c-1].'</td>';
                if ($Note1Description[$Effect]<>"") {
                  echo '<td>'.$Note1[$Effect][$c-1].'</td>';
                }
                if ($Note2Description[$Effect]<>"") {
                  echo '<td>'.$Note2[$Effect][$c-1].'</td>';
                }
                if ($Note3Description[$Effect]<>"") {
                  echo '<td>'.$Note3[$Effect][$c-1].'</td>';
                }
                echo '</tr>';
                $c = $c+1;
              }
          ...
          //if not directed to a specific effect, show the table
          else {
            echo '<table border="1" cellpadding="3" style="border-collapse: collapse">';
            foreach ($Name as $value) {
              echo '<tr>';
              echo '<td>'.keywords($value, $KeywordName, $KeywordCode, $KeywordLink).'</td>';
              echo '<td>'.keywords($ShortDescription[$value], $KeywordName, $KeywordCode, $KeywordLink).'</td>';
              echo '</tr>';
            }
            echo '</table>';
          }
