  ...
  <div class="holy-grail-grid">
      ...
      <div class="holy-grail-middle">
        ...
        <?php
          ...
          //Parses "CharacterCreation" Excel doc
          if ( $xlsx = SimpleXLSX::parse('Combat.xlsx') ) {
            foreach( $xlsx->rows() as $r ) {
              switch ($r[0]) {
                case "Section":
                  echo '<p><b>'.keywords($r[1], $KeywordName, $KeywordCode, $KeywordLink).'</b></p>';
                  break;
                case "TableStart":
                  echo '<table border="1" cellpadding="3" style="border-collapse: collapse; margin-left:10%; width: 80%;"><tr><td>'.$r[2].'</td><td>'.$r[3].'</td><td>'.$r[4].'</td><td>'.$r[5].'</td></tr>';
                  break;
                case "Table":
                  echo '<tr><td>'.$r[2].'</td><td>'.$r[3].'</td><td>'.$r[4].'</td><td>'.$r[5].'</td></tr>';
                  break;
                case "TableEnd":
                  echo '<tr><td>'.$r[2].'</td><td>'.$r[3].'</td><td>'.$r[4].'</td><td>'.$r[5].'</td></tr></table>';
                  break;
                case "italic":
                  echo '<p style="margin-left:10%; margin-right:10%;"><i>'.keywords($r[1], $KeywordName, $KeywordCode, $KeywordLink).'</i></p>';
                  break;
                case "bullet":
                  echo '<p style="margin-left:10%; margin-right:10%;">'.keywords($r[1], $KeywordName, $KeywordCode, $KeywordLink).'</p>';
                  break;
                case "text":
                  echo '<p>'.keywords($r[1], $KeywordName, $KeywordCode, $KeywordLink).'</p>';
                  break;
                default:
                  // default
              }
