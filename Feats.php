  ...
  <div class="holy-grail-grid">
      ...
      <div class="holy-grail-middle">
        ...
        <?php
          ...
          //Parses "Feats" Excel doc
          if ( $xlsx = SimpleXLSX::parse('Feats.xlsx') ) {
            foreach( $xlsx->rows() as $r ) {
              $FeatNum[$r[0]]=$r[1];
              $FeatType[$r[1]]=$r[2];
              $FeatDesc[$r[1]]=$r[3];
              $FeatShortDesc[$r[1]]=$r[4];
              $FeatReqs[$r[1]]=$r[5];
              $FeatLevel[$r[1]]=$r[6];
              $FeatAbility[$r[1]]=$r[7];
              $FeatAbilityReq[$r[1]]=$r[8];
              $FeatNotes[$r[1]]=$r[9];
              $FeatCritical[$r[1]]=$r[10];
              $FeatStance[$r[1]]=$r[11];
            }
          }
          else {
          ...
          if ($_GET) {
              ...
              echo '<p><i>'.keywords($FeatNotes[$Feat], $KeywordName, $KeywordCode, $KeywordLink).'</i></p>';
          }
          //if not directed to a specific feat, show the table
          else {
            echo '<table border="1" cellpadding="3" style="border-collapse: collapse" id="FeatsTable">';
            foreach ($FeatNum as $value) {
              If($value==='Name'){
                ...
                echo '<th onclick="sortTable(2)">'.keywords($FeatShortDesc[$value], $KeywordName, $KeywordCode, $KeywordLink).'</th>';
                ...
                echo '<td>'.keywords($FeatShortDesc[$value], $KeywordName, $KeywordCode, $KeywordLink).'</td>';
